        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'reduxframework' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'reduxframework' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'reduxframework' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'reduxframework' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'reduxframework'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'reduxframework' );
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    load_plugin_textdomain( 'reduxframework', '', self::$_dir . 'languages/' . $locale . '.mo' );
                } else {
                    load_textdomain( 'reduxframework', self::$_dir . 'languages/' . $locale . '.mo' );
                }
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'reduxframework' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'reduxframework' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'reduxframework' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'reduxframework' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'reduxframework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'reduxframework' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'reduxframework' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'reduxframework' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'reduxframework' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'reduxframework' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = esc_html__('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'reduxframework');
                    ...
                    if (!empty($this->args['share_icons'])) {
                        foreach($this->args['share_icons'] as $idx => $arr) {
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = esc_html__('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'reduxframework');
