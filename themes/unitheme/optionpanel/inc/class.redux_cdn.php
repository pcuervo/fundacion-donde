                    ...
                    if ( is_wp_error( $cdn_response ) || wp_remote_retrieve_response_code( $cdn_response ) != '200' ) {
                        if ( class_exists( 'Redux_VendorURL' ) ) {
                            ...
                            if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
                                $msg = esc_html__( 'Please wait a few minutes, then try refreshing the page. Unable to load some remotely hosted scripts.', 'reduxframework' );
                                if ( self::$_parent->args['dev_mode'] ) {
                                    $msg = sprintf( esc_html__( 'If you are developing offline, please download and install the <a href="%s" target="_blank">Redux Vendor Support</a> plugin/extension to bypass the our CDN and avoid this warning', 'reduxframework' ), 'https://github.com/reduxframework/redux-vendor-support' );
                                }

                                self::$_parent->admin_notices[] = array(
                                    'type'    => 'error',
                                    'msg'     => '<strong>' . esc_html__( 'Redux Framework Warning', 'reduxframework' ) . '</strong><br/>' . sprintf( esc_html__( '%s CDN unavailable.  Some controls may not render properly.', 'reduxframework' ), $handle ) . '  ' . $msg,
                                    'id'      => $handle . $tran_key,
                                    'dismiss' => false,
                                );
